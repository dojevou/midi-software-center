<script lang="ts">
  ...
  async function handleCreate() {
    ...
    try {
      ...
      await Vip3BrowserApi.createCollection({
        name: name.trim(),
        description: description.trim() || undefined,
        icon: icon || undefined,
        color: color || undefined,
        is_smart: isSmart || undefined,
        smart_filters: isSmart ? currentFilters : undefined,
      });
  ...
  function getFilterSummary(filters: Vip3Filters): string {
    const parts: string[] = [];
...
<div class="modal-overlay" on:click={onClose}>
  <div class="modal" on:click|stopPropagation>
    ...
    <div class="modal-body">
      ...
      <div class="form-group">
        ...
        <input
          ...
          type="text"
          bind:value={name}
          placeholder="e.g., My Favorite Loops"
          maxlength="100"
          disabled={saving}
          autofocus
        />
      ...
      {#if isSmart}
        <div class="smart-filters-info">
          <strong>Smart Collection Filters:</strong>
          <div class="filter-summary">
            {#if currentFilters.folder_ids?.length || currentFilters.instrument_ids?.length || currentFilters.timbre_ids?.length || currentFilters.style_ids?.length}
              ...
              {/if}
            {:else}
              <span class="no-filters">No filters applied - collection will include all files</span>
            {/if}
          </div>
        </div>
      {/if}
    ...
    <div class="modal-footer">
      ...
      <button class="btn-primary" on:click={handleCreate} disabled={saving || !name.trim()}>
