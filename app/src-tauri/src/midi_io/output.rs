...
impl MidiOutputHandle {
    ...
    pub fn new() -> Arc<Self> {
        ...
        Arc::new(Self { command_tx: tx, running, messages_sent: AtomicU64::new(0) })
    }
    ...
    pub fn list_devices(&self) -> Result<Vec<MidiOutputDevice>, String> {
        ...
        self.command_tx
            .send(OutputCommand::ListDevices(tx))
    ...
    pub fn open_device(&self, device_id: &str) -> Result<(), String> {
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::OpenDevice(device_id.to_string(), tx))
            .map_err(|_| "MIDI thread not running".to_string())?;
        rx.blocking_recv().map_err(|_| "Channel closed".to_string())?
    }
    ...
    pub fn close_device(&self, device_id: &str) -> Result<(), String> {
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::CloseDevice(device_id.to_string(), tx))
            .map_err(|_| "MIDI thread not running".to_string())?;
        rx.blocking_recv().map_err(|_| "Channel closed".to_string())?
    }
    ...
    pub fn send_program(&self, device_id: &str, channel: u8, program: u8) -> Result<(), String> {
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::SendProgram(
                device_id.to_string(),
                channel,
                program,
                tx,
    ...
    pub fn send_pitch_bend(&self, device_id: &str, channel: u8, value: i16) -> Result<(), String> {
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::SendPitchBend(
                device_id.to_string(),
                channel,
                value,
                tx,
    ...
    pub fn send_aftertouch(
        ...
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::SendAftertouch(
                device_id.to_string(),
                channel,
                pressure,
                tx,
    ...
    pub fn send_poly_aftertouch(
        ...
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::SendPolyAftertouch(
                device_id.to_string(),
                channel,
                note,
                pressure,
    ...
    pub fn send_clock(&self, device_id: &str) -> Result<(), String> {
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::SendClock(device_id.to_string(), tx))
            .map_err(|_| "MIDI thread not running".to_string())?;
        rx.blocking_recv().map_err(|_| "Channel closed".to_string())?
    }
    ...
    pub fn send_start(&self, device_id: &str) -> Result<(), String> {
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::SendStart(device_id.to_string(), tx))
            .map_err(|_| "MIDI thread not running".to_string())?;
        rx.blocking_recv().map_err(|_| "Channel closed".to_string())?
    }
    ...
    pub fn send_continue(&self, device_id: &str) -> Result<(), String> {
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::SendContinue(device_id.to_string(), tx))
            .map_err(|_| "MIDI thread not running".to_string())?;
        rx.blocking_recv().map_err(|_| "Channel closed".to_string())?
    }
    ...
    pub fn send_stop(&self, device_id: &str) -> Result<(), String> {
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::SendStop(device_id.to_string(), tx))
            .map_err(|_| "MIDI thread not running".to_string())?;
        rx.blocking_recv().map_err(|_| "Channel closed".to_string())?
    }
    ...
    pub fn send_bank_select(
        ...
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::SendBankSelect(
                device_id.to_string(),
                channel,
                bank_msb,
                bank_lsb,
    ...
    pub fn send_all_notes_off(&self, device_id: &str, channel: u8) -> Result<(), String> {
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::SendAllNotesOff(
                device_id.to_string(),
                channel,
                tx,
            ))
    ...
    pub fn send_all_notes_off_all_channels(&self, device_id: &str) -> Result<(), String> {
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::SendAllNotesOffAllChannels(
                device_id.to_string(),
                tx,
            ))
            .map_err(|_| "MIDI thread not running".to_string())?;
    ...
    pub fn send_all_sounds_off(&self, device_id: &str, channel: u8) -> Result<(), String> {
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::SendAllSoundsOff(
                device_id.to_string(),
                channel,
                tx,
            ))
    ...
    pub fn send_all_sounds_off_all_channels(&self, device_id: &str) -> Result<(), String> {
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::SendAllSoundsOffAllChannels(
                device_id.to_string(),
                tx,
            ))
            .map_err(|_| "MIDI thread not running".to_string())?;
    ...
    pub fn send_reset_all_controllers(&self, device_id: &str, channel: u8) -> Result<(), String> {
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::SendResetControllers(
                device_id.to_string(),
                channel,
                tx,
            ))
    ...
    pub fn send_reset_all_controllers_all_channels(&self, device_id: &str) -> Result<(), String> {
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::SendResetControllersAllChannels(
                device_id.to_string(),
                tx,
            ))
            .map_err(|_| "MIDI thread not running".to_string())?;
    ...
    pub fn send_panic(&self, device_id: &str) -> Result<(), String> {
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::SendPanic(device_id.to_string(), tx))
            .map_err(|_| "MIDI thread not running".to_string())?;
        rx.blocking_recv().map_err(|_| "Channel closed".to_string())?
    }
    ...
    pub fn send_panic_all(&self) -> Result<(), String> {
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::SendPanicAll(tx))
            .map_err(|_| "MIDI thread not running".to_string())?;
        rx.blocking_recv().map_err(|_| "Channel closed".to_string())?
    }
    ...
    pub fn schedule_message(&self, message: ScheduledMessage) -> Result<(), String> {
        let (tx, rx) = oneshot::channel();
        self.command_tx
            .send(OutputCommand::ScheduleMessage(message, tx))
            .map_err(|_| "MIDI thread not running".to_string())?;
        rx.blocking_recv().map_err(|_| "Channel closed".to_string())?
    }
...
fn run_output_thread(rx: std::sync::mpsc::Receiver<OutputCommand>, running: Arc<AtomicBool>) {
...
fn process_command(
    ...
    match cmd {
        ...
        OutputCommand::SendBankSelect(
            ...
            match mode {
                BankSelectMode::Standard | BankSelectMode::Proteus2000 => {
                    result = send_raw_internal(
                        connections,
                        &device_id,
                        &[0xB0 | ch, 0, bank_msb & 0x7F],
                        start_time,
                    );
                    if result.is_ok() {
                        result = send_raw_internal(
        ...
        OutputCommand::SendAllNotesOff(device_id, channel, tx) => {
            let ch = channel.min(15);
            let result =
                send_raw_internal(connections, &device_id, &[0xB0 | ch, 123, 0], start_time);
            let _ = tx.send(result);
        },
        ...
        OutputCommand::SendAllSoundsOff(device_id, channel, tx) => {
            let ch = channel.min(15);
            let result =
                send_raw_internal(connections, &device_id, &[0xB0 | ch, 120, 0], start_time);
            let _ = tx.send(result);
        },
        ...
        OutputCommand::SendResetControllers(device_id, channel, tx) => {
            let ch = channel.min(15);
            let result =
                send_raw_internal(connections, &device_id, &[0xB0 | ch, 121, 0], start_time);
            let _ = tx.send(result);
        },
        ...
        OutputCommand::ScheduleMessage(msg, tx) => {
            *sequence_counter += 1;
            schedule_queue.push(ScheduledEntry { message: msg, sequence: *sequence_counter });
            let _ = tx.send(Ok(()));
        },

        OutputCommand::CancelScheduled(device_id, tx) => {
            let remaining: Vec<_> =
                schedule_queue.drain().filter(|e| e.message.device_id != device_id).collect();
...
fn process_scheduled_message(
    ...
    let data = match msg.message {
        ...
        MidiOutputMessageType::ControlChange { controller, value } => {
            vec![0xB0 | ch, controller & 0x7F, value & 0x7F]
        },
        ...
        MidiOutputMessageType::PolyAftertouch { note, pressure } => {
            vec![0xA0 | ch, note & 0x7F, pressure & 0x7F]
        },
...
fn send_raw_internal(
    ...
    wrapper.connection.send(data).map_err(|e| format!("Send failed: {}", e))?;
...
pub fn midi_send_bank_select(
    ...
    state
        .0
        .send_bank_select(&device_id, channel, bank_msb, bank_lsb, program, bank_mode)
}
...
#[tauri::command]
pub fn midi_output_get_all_statuses(state: State<MidiOutputSendState>) -> Vec<OutputDeviceStatus> {
...
mod tests {
    ...
    fn test_parse_manufacturer_proteus() {
        ...
        assert_eq!(
            parse_manufacturer("Proteus 2000 Port 1"),
            Some("E-mu".into())
        );
    }
