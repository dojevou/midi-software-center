...
use crate::db::repositories::pipeline::tag_repository::TagRepository;
use crate::AppState;
use tauri::State;
...
/// Instrument categories for VIP3 browser
const INSTRUMENT_CATEGORIES: &[&str] = &[
    "drums",
    "bass",
    "keys",
    "guitar",
...
/// Get all instrument tags (combines multiple categories)
#[tauri::command]
pub async fn get_all_instruments(state: State<'_, AppState>) -> Result<Vec<TagResponse>, String> {
    let pool = state.database.pool().await;
    let repo = TagRepository::new(pool);

    let categories: Vec<String> = INSTRUMENT_CATEGORIES.iter().map(|s| s.to_string()).collect();

    let tags = repo
        .get_tags_by_categories(&categories)
        .await
...
/// Get all mood/timbre tags for VIP3 browser
#[tauri::command]
pub async fn get_vip3_moods(state: State<'_, AppState>) -> Result<Vec<TagResponse>, String> {
    let pool = state.database.pool().await;
    let repo = TagRepository::new(pool);

    let tags = repo
        .get_tags_by_category("mood")
        .await
        .map_err(|e| format!("Failed to get moods: {}", e))?;

    Ok(tags.into_iter().map(TagResponse::from).collect())
}

/// Get all musical key tags for VIP3 browser
#[tauri::command]
pub async fn get_vip3_keys(state: State<'_, AppState>) -> Result<Vec<TagResponse>, String> {
    let pool = state.database.pool().await;
    let repo = TagRepository::new(pool);

    let tags = repo
        .get_tags_by_category("key")
        .await
