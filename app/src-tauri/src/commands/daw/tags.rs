...
#[tauri::command]
pub async fn delete_tag(tag_id: i32, state: State<'_, DawAppState>) -> Result<(), String> {
    let pool = state.db_pool.as_ref().ok_or("Database not connected")?;
...
#[tauri::command]
pub async fn export_tags_csv(state: State<'_, DawAppState>) -> Result<String, String> {
    let pool = state.db_pool.as_ref().ok_or("Database not connected")?;
...
#[tauri::command]
pub async fn get_tag(tag_id: i32, state: State<'_, DawAppState>) -> Result<Tag, String> {
    let pool = state.db_pool.as_ref().ok_or("Database not connected")?;
...
/// Get all tags (DAW)
#[tauri::command]
pub async fn daw_get_all_tags(state: State<'_, DawAppState>) -> Result<Vec<Tag>, String> {
    let pool = state.db_pool.as_ref().ok_or("Database not connected")?;
