...
#[command]
pub async fn database_remove_file(state: State<'_, DawAppState>, id: i64) -> Result<(), String> {
    let pool = state.db_pool.as_ref().ok_or("Database not available".to_string())?;
...
#[command]
pub async fn database_get_stats(state: State<'_, DawAppState>) -> Result<DatabaseStats, String> {
    let pool = state.db_pool.as_ref().ok_or("Database not available".to_string())?;
